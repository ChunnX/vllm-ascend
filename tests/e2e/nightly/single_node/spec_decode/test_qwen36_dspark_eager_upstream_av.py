...
"""Opt-in 910B4 TP=4 model gate for eager lane B (upstream AV manager).

Runs the real upstream AdaptiveVerificationManager eager -- cost-argmax budget,
device survival top-k, async D2H -- with a synthetic cost curve, and checks its
greedy output matches fixed-K decoding. Each mode owns a fresh engine process.
"""
...
def test_greedy_eager_upstream_av_matches_fixed_k(tmp_path):
...
"""
    ...
    def run(adaptive):
        ...
        key = "VLLM_ASCEND_DSPARK_EAGER_UPSTREAM_AV"
        env.pop(key, None)
        if adaptive:
            env[key] = "1"
        ...
        if adaptive:
            assert "DSpark eager upstream AV active" in log
