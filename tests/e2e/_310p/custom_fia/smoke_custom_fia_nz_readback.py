...
"""Phase 0 hardware gate: custom FIA must read vLLM's NZ paged cache directly.

The whole 310P adapter is built on one assumption -- that the kernel can consume
the cache vLLM already allocated and wrote, with no gather, no NZ->ND conversion and
...
Deliberately not a pytest test: it needs real 310P hardware and a vllm-ascend
built for a 310P SOC, which CI hosts do not have. Run it by hand:

    python tests/e2e/_310p/custom_fia/smoke_custom_fia_nz_readback.py
...
* ``scale = head_dim ** -0.5``, matching both production and the operator-level
  tests in ``Ascend_Ops/tests/test_adn_fia.py`` (the upstream name of this op).
...
the kernel reads *that* correctly is exactly what has to be proven here, and
nothing upstream proves it.
"""
...
def main():
    ...
    if FAILURES:
        ...
        print(
            ...
            "writer layout disagrees with the kernel; correct shapes but wrong numbers usually "
            "means num_key_value_heads, block_size or scale. Do not add a repack to the "
            "hot path to work around it."
        )
        sys.exit(1)
    print("PASS: the kernel reads vLLM's NZ cache directly, and the result is non-causal.")
