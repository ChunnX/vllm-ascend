...
| Phase 0 硬件门禁（ADN NZ readback / 输入展开 op smoke） | 🟡 脚本就绪，待真机 | **需 310P 真机 + ADN** |
...
⚠️ **`ATOL`/`RTOL` 现在是占位值 5e-3**，必须换成 Phase 0.2 从 ATK 结果 JSON 或框架配置
读到的真实判据（YAML 只写 `standard: acc: default`，没有字面数值）。
