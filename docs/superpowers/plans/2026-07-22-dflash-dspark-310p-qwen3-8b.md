...
| Task 4 patch import | 与 RoPE 合并 | 两个 Qwen3 patch 在 310P 加载；测试 monkey-patch identity，不能用本来就为真的 `hasattr` |
...
（`dflash_proposer.py:229-235`、`dspark_proposer.py:178-184`）：
...
`set_cos_and_sin` 在 `worker/model_runner_v1.py:480`（runner `__init__`）分配，
...
`model_runner_v1.py:2293` / `:3618` 刷新过 slice。因此 drafter profile 读到的是
...
| `proposer.kernel_block_size` | 标量 | `llm_base_proposer.py:337`，`get_supported_kernel_block_sizes()[0]`，未经 `head_size` 过滤 | 可选窄范围 guard，断言 `== 128` |
...
（`llm_base_proposer.py:1679` 那处 `if self.has_gdn: block_size = self.kernel_block_size` 是另一条
...
| `vllm_ascend/patch/worker/__init__.py` | 310P 也加载 Qwen3 DFlash/DSpark 两个 patch |
...
9. 使用 Phase 0.2 从 ATK 结果/框架配置抄出来的实际判据，同时打印 max/mean absolute error；不得拍脑袋固定
   `max_abs < 5e-3`。
...
调用时从 `runner.input_batch.block_table[kv_cache_gid].block_size` 读取实际标量。按 §3.1 的权威
来源声明，这是本期唯一权威值。三个字段类型不同，guard 也要分开写：

```python
block_size = self.runner.input_batch.block_table[self.kv_cache_gid].block_size
if block_size != 128:
    raise RuntimeError(
        f"this plan only covers kernel block size 128, got {block_size}; "
        f"re-scope before changing anything"
    )

# Narrow-scope guard. kernel_block_size still feeds the non-310P Triton launcher's
# slot mapping, so the two agreeing is a property of this scope, not a contract.
if self.kernel_block_size != block_size:
    raise RuntimeError(
        f"proposer.kernel_block_size ({self.kernel_block_size}) diverged from the "
        f"BlockTable's selected size ({block_size}). See the authority table in "
        f"section 3.1 -- this needs a scope decision, not a local fix."
    )

# runner.kernel_block_sizes[gid] is a *candidate list* (typically [128, 64]),
# never a scalar. Check membership of the selected size, not equality.
candidates = self.runner.kernel_block_sizes[self.kv_cache_gid]
if block_size not in candidates:
    raise RuntimeError(
        f"selected block size {block_size} is not in the runner's candidate list "
        f"{candidates} for KV cache group {self.kv_cache_gid}"
    )
```
...
```python
        self._expand_drafting_inputs(
            ...
            out_context_slot_mapping=self._context_slot_mapping_buffer,
...
**DSpark 调用点**（替换 `dspark_proposer.py` 里原来的 kernel launch）。除了两个参数以外与上面逐字
相同——`num_query_per_req` 这里是 `self.num_speculative_tokens`（即 K=7，不是 K+1），
`sample_from_anchor=True` 让采样从 anchor 位置起取 K 个：
...
不要把 DSpark 简化成"query 数不同的 DFlash"——这两个参数是它们唯一的语义差别，写错不会报错，
只会让 acceptance 悄悄变差。同时删掉 `dspark_proposer.py:9` 那行不再使用的 Triton import。
...
class TestProposerDispatch(TestBase):
    def _make_proposer(self, cls, num_spec, q_per_req):
        ...
        proposer._context_slot_mapping_buffer = torch.zeros(64, dtype=torch.int32)
    ...
    def _run(self, cls, num_spec, q_per_req, module, on_310p):
        ...
        with (
            mock_patch(f"{module}.is_310p", return_value=on_310p),
            mock_patch(
            ...
            mock_patch(
                f"{module}.copy_and_expand_dflash_and_dspark_inputs_kernel_single_grid",
                _ExplodingLauncher(),
            ),
        ):
    ...
    def test_310p_dflash_uses_helper_with_skip_anchor(self):
        ...
        seen, _, _ = self._run(
            AscendDflashProposer, 8, 9, "vllm_ascend.spec_decode.dflash_proposer", on_310p=True
        )
    ...
    def test_310p_dspark_uses_same_helper_with_anchor_sampling(self):
        ...
        seen, _, _ = self._run(
            AscendDsparkProposer, 7, 7, "vllm_ascend.spec_decode.dspark_proposer", on_310p=True
        )
    ...
    def test_non_310p_still_launches_triton(self):
        ...
        with self.assertRaisesRegex(AssertionError, "launched the Triton kernel"):
            self._run(
                AscendDflashProposer, 8, 9, "vllm_ascend.spec_decode.dflash_proposer", on_310p=False
            )
...
把以下两个 import 移出 `if not is_310p()`：

- `patch_qwen3_dflash`；
- `patch_qwen3_dspark`。

`patch_qwen3_5` 和 `patch_qwen3vl` 继续保持 310P gate，不扩大模型范围。
...
（`qwen3_dflash.py:474-477`）。逐层写法保持同样语义但不依赖那个假设，代价为零。
...
- [ ] 在 subprocess 中断言 DFlash method identity 指向 patch 实现；
- [ ] 在 subprocess 中断言 DSpark `_init_parallel_drafting_params` identity 指向 patch 实现；
- [ ] DSpark 从 checkpoint config 读取正确 mask token ID。
...
class TestProfileRopeFlagCoverage(TestBase):
    ...
    def _run_profile(self, proposer_cls, num_spec, q_per_req):
        from unittest.mock import patch as mock_patch
        ...
        with (
            mock_patch("vllm_ascend.utils.is_310p", return_value=True),
            mock_patch("vllm_ascend.spec_decode.dflash_proposer.is_310p", return_value=True),
            mock_patch("vllm_ascend.spec_decode.llm_base_proposer.set_ascend_forward_context"),
            mock_patch("vllm_ascend.spec_decode.llm_base_proposer.get_forward_context"),
        ):
    ...
    def test_dflash_profile_covers_context_and_query(self):
        ...
        self._assert_both_forwards_covered(self._run_profile(AscendDflashProposer, 8, 9))

    def test_dspark_profile_covers_context_and_query(self):
        from vllm_ascend.spec_decode.dspark_proposer import AscendDsparkProposer

        self._assert_both_forwards_covered(self._run_profile(AscendDsparkProposer, 7, 7))
...
_ADN_MAX_HEAD_DIM = 256
_ADN_MAX_HEAD_DIM_X_BLOCK = 16384
_ADN_MAX_GQA_RATIO = 64
...
def validate_adn_capability(*, query, key_cache, value_cache, num_heads, num_kv_heads, head_size, block_size):
    ...
    if key_cache.shape[-2] != block_size:
        raise RuntimeError(f"cache block size {key_cache.shape[-2]} != {block_size}")
    ...
    if not 0 < head_size <= _ADN_MAX_HEAD_DIM:
        raise RuntimeError(f"ADN requires 0 < head_dim <= {_ADN_MAX_HEAD_DIM}, got {head_size}")
    if block_size <= 0 or block_size % 16:
        raise RuntimeError(f"ADN requires block_size > 0 and a multiple of 16, got {block_size}")
    if head_size * block_size > _ADN_MAX_HEAD_DIM_X_BLOCK:
        raise RuntimeError(
            f"head_dim*block_size = {head_size * block_size} exceeds {_ADN_MAX_HEAD_DIM_X_BLOCK}"
        )
    if num_kv_heads <= 0 or num_heads % num_kv_heads:
        raise RuntimeError(f"invalid GQA layout: {num_heads} Q heads / {num_kv_heads} KV heads")
    if num_heads // num_kv_heads > _ADN_MAX_GQA_RATIO:
        raise RuntimeError(
            f"GQA ratio {num_heads // num_kv_heads} exceeds ADN's {_ADN_MAX_GQA_RATIO}"
        )
...
def forward_parallel_draft_adn(self, query, attn_metadata, output):
    ...
    for b, (ql, kl) in enumerate(zip(q_lens, kv_lens)):
        ...
        if kl > capacity:
            raise RuntimeError(
                ...
                f"({block_table.shape[1]} pages x {block_size})"
            )
    ...
    if adn_out.dtype != output_slice.dtype or adn_out.numel() != output_slice.numel():
        raise RuntimeError(
            f"ADN returned {tuple(adn_out.shape)}/{adn_out.dtype}, incompatible with "
            f"the output slice {tuple(output_slice.shape)}/{output_slice.dtype}"
        )
