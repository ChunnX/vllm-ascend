...
#ifndef CUSTOM_FUSED_INFER_ATTENTION_TORCH_ADPT_H
#define CUSTOM_FUSED_INFER_ATTENTION_TORCH_ADPT_H
...
inline void run_custom_fused_infer_attention(
    ...
    at::Tensor& output)
{
    TORCH_CHECK(key.size() == 1, "custom FIA expects exactly one paged key-cache tensor, got ", key.size());
    TORCH_CHECK(value.size() == 1, "custom FIA expects exactly one paged value-cache tensor, got ", value.size());
    TORCH_CHECK(
        actual_seq_lengths_q.has_value() && !actual_seq_lengths_q->empty(),
        "custom FIA requires non-empty actual_seq_lengths_q");
    TORCH_CHECK(
        actual_seq_lengths_kv.has_value() && !actual_seq_lengths_kv->empty(),
        "custom FIA requires non-empty actual_seq_lengths_kv");
    TORCH_CHECK(
        actual_seq_lengths_q->size() == actual_seq_lengths_kv->size(),
        "custom FIA q/kv sequence-length batches differ: ",
        actual_seq_lengths_q->size(),
        " vs ",
        actual_seq_lengths_kv->size());
    TORCH_CHECK(
        block_table.has_value() && block_table->defined(),
        "custom FIA paged attention requires block_table");
    TORCH_CHECK(
        output.sizes().equals(query.sizes()),
        "custom FIA output shape must match query shape, got ",
        output.sizes(),
        " vs ",
        query.sizes());
    TORCH_CHECK(
        output.scalar_type() == query.scalar_type(),
        "custom FIA output dtype must match query dtype, got ",
        output.scalar_type(),
        " vs ",
        query.scalar_type());
    TORCH_CHECK(
        output.device() == query.device(),
        "custom FIA output device must match query device, got ",
        output.device(),
        " vs ",
        query.device());
    ...
    EXEC_NPU_CMD(
        aclnnCustomFusedInferAttention,
        query,
        key,
        value,
        attn_mask,
...
at::Tensor npu_custom_fused_infer_attention(
    const at::Tensor& query,
    at::TensorList key,
    at::TensorList value,
    const c10::optional<at::Tensor>& attn_mask,
    ...
    int64_t inner_precise)
{
    ...
    detail::run_custom_fused_infer_attention(
        query,
        key,
        value,
        attn_mask,
...
at::Tensor npu_custom_fused_infer_attention_out(
    const at::Tensor& query,
    at::TensorList key,
    at::TensorList value,
    at::Tensor& output,
