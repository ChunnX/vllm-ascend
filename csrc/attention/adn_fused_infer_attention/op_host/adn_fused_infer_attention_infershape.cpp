...
/*!
 * \file adn_fused_infer_attention_proto.cpp
 * \brief
 */
...
{
    OPS_LOG_D(context->GetNodeName(), "Enter AdnFusedInferAttention inferShape impl.");
    // query shape
    ...
    *attentionOutShape = *queryShape;
    ...
    OPS_LOG_D(context->GetNodeName(), "AdnFusedInferAttention inferShape end.");
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InferDataTypeIncreFlashAttention(gert::InferDataTypeContext *context)
{
    OPS_LOG_D(context->GetNodeName(), "Enter AdnFusedInferAttention inferDataType impl.");
    context->SetOutputDataType(IFA_ATTENTION_OUT_INDEX, ge::DT_FLOAT16);
    OPS_LOG_D(context->GetNodeName(), "AdnFusedInferAttention inferDataType end.");
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(AdnFusedInferAttention)
