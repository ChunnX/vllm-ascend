# ADN fused infer attention for Ascend 310P

This directory vendors the 310P `AdnFusedInferAttention` OPP from
...
- `op_host/`: OpDef, shape/type inference, tiling, checks, and tiling-key
  registration.
- `adn_fused_infer_attention_torch_adpt.h`: ACLNN-to-Torch functional and
...
```python
out = torch.ops._C_ascend.npu_adn_fused_infer_attention(
    query,
    [key_cache],
    [value_cache],
    attn_mask=None,
